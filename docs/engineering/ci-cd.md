...
本文档描述 `fe-spec` 项目（前端编码规范工程化）的 CI/CD 体系，包括本地开发校验、GitHub Actions 自动化构建部署、npm 包发布等完整流程。
...
│                         本地开发流程                              │
│                                                                 │
│  git commit ──► pre-commit hook ──► commit-msg hook             │
│       │                              │                           │
│       │                        commitlint 校验                  │
│       │                              │                           │
│       ▼                              ▼                           │
...
│                                                                 │
│  build job:                                                     │
│    checkout ──► setup pnpm ──► setup node@18 ──► install ──►     │
│    docs:build ──► upload artifact                               │
│                                                                 │
│  deploy job:                                                    │
...
项目强制使用 `pnpm`，根目录 [package.json](https://github.com/chenweita/fe-spec/blob/main/package.json) 中配置了：
...
| Hook | 触发时机 | 作用 |
|------|---------|------|
| `pre-commit` | `git commit` 执行前 | 预留位置（可扩展：ESLint、stylelint、markdownlint 等） |
...
文件位置：[.github/workflows/deploy.yml](https://github.com/chenweita/fe-spec/blob/main/.github/workflows/deploy.yml)
...
[lerna.json](https://github.com/chenweita/fe-spec/blob/main/lerna.json) 中的关键配置：
...
## 5. 手动部署脚本（已被 GitHub Actions 取代）

项目根目录的 [deploy.sh](https://github.com/chenweita/fe-spec/blob/main/deploy.sh) 是早期的手动部署脚本，将文档构建产物推送到 `gh-pages` 分支：
...
git push -f $push_addr HEAD:gh-pages
```

> ⚠️ **该脚本已被 GitHub Actions 原生 Pages 部署取代**，不再推荐使用。保留仅作历史参考。
